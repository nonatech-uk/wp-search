<?php
/**
 * GitHub Plugin Updater for Search
 *
...
class Search_GitHub_Updater {
     ...
     * @param string $repo GitHub repository (e.g., 'nonatech-uk/wp-search')
    ...
    public function __construct($plugin_file, $repo, $current_version) {
        ...
        $this->cache_key = 'search_github_update_' . md5($repo);
    ...
    public function add_check_update_link($links) {
        $url = wp_nonce_url(admin_url('plugins.php?search_check_update=1'), 'search_check_update');
    ...
    public function handle_check_update() {
        if (empty($_GET['search_check_update'])) {
            return;
        }

        if (!current_user_can('update_plugins') || !wp_verify_nonce($_GET['_wpnonce'], 'search_check_update')) {
            return;
        }
